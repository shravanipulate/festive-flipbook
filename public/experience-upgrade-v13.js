(() => {
  ...
  // Final-page reactions: each reaction is an independent toggle.
  // ON  = glowing/selected
  // OFF = normal/unlit
  // We deliberately keep the existing final-page markup and Neech joke intact.
    ...
    style.textContent = `
      #finalReactions .vault-reaction[data-reaction]:not([data-reaction="Neech"]) {
        ...
        transition:transform .2s ease, background .2s ease, border-color .2s ease,
          color .2s ease, filter .2s ease, box-shadow .2s ease;
      }
      ...
      #finalReactions .vault-reaction[data-reaction]:not([data-reaction="Neech"]).reaction-on .vr-icon {
        filter:drop-shadow(0 0 8px rgba(255,255,255,.32));
      }
      #finalReactions .vault-reaction[data-reaction]:not([data-reaction="Neech"]).reaction-off {
        transform:none;
        box-shadow:none;
        filter:none;
      }
  ...
  function getButtons() {
    return [...document.querySelectorAll('#finalReactions .vault-reaction[data-reaction]')]
      .filter(btn => btn.dataset.reaction !== 'Neech');
  }
  ...
  function syncStatus(btn, on) {
    ...
    status.textContent = on
      ? '✓ Reacted: ' + btn.dataset.reaction
      : '○ ' + btn.dataset.reaction + ' unselected';
  }
  ...
  function patchButtons() {
    ...
    getButtons().forEach(btn => {
      ...
      const initiallyOn = btn.classList.contains('selected');
      setReaction(btn, initiallyOn);
    });
  }

  // Capture-phase handler runs before the old inline onclick handlers, so the old
  // "only one reaction can be selected" behaviour cannot immediately undo the toggle.
