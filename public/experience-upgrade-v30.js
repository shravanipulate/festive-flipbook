(() => {
  ...
  function existingSearchButton(input) {
    ...
    for (let i = 0; n && i < 7; i++, n = n.parentElement) {
      ...
      const b = buttons.find(x => /^\s*search\s*$/i.test(x.textContent || ''))
        || buttons.find(x => /search/i.test(x.getAttribute('aria-label') || ''));
      if (b) return b;
    }
    return null;
  }
  ...
  function statusFor(block) {
    ...
    return all.find(el => /type a song name first|searching youtube|results|playing selected/i.test(String(el.textContent || '').trim())) || null;
  }
  ...
  async function playVideo(videoId, title, status, host) {
    ...
    if (player && typeof player.loadVideoById === 'function') {
      ...
      setStatus(status, `▶ Playing: ${title}`);
      return;
    }
    ...
    player = new YT.Player(host, {
      ...
      events: {
        onReady: e => {
          try { e.target.setVolume(70); e.target.unMute(); e.target.playVideo(); } catch (_) {}
          setStatus(status, `▶ Playing: ${title}`);
        },
        onStateChange: e => {
          if (window.YT && e.data === YT.PlayerState.PLAYING) setStatus(status, `▶ Playing: ${title}`);
        },
  ...
  function addResult(results, video, status, host) {
    ...
    row.addEventListener('click', async () => {
      try {
        await playVideo(video.id, video.title, status, host);
      } catch (err) {
        console.error('[birthday YouTube player]', err);
        setStatus(status, `⚠️ Could not load player: ${err?.message || 'try again'}`);
      }
    });
  ...
  function wire() {
    ...
    input.addEventListener('input', () => {
      ...
      // Don't spend a YouTube quota unit on every keystroke.
