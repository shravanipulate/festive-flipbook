(() => {
  ...
  style.textContent = `
    #unscramble-overlay{position:fixed;inset:0;z-index:1000005;display:none;align-items:center;justify-content:center;padding:20px;background:rgba(8,7,7,.97);backdrop-filter:blur(16px);color:#fff;font-family:inherit}
    #unscramble-overlay.show{display:flex}
    ...
    .us-kicker{font-size:9px;letter-spacing:.28em;text-transform:uppercase;opacity:.45}
    .us-title{font-size:27px;font-weight:850;margin:0}
    .us-copy{font-size:12px;line-height:1.65;opacity:.58;margin:10px auto 22px;max-width:570px}
    .us-board{min-height:155px;border:1px solid rgba(255,255,255,.1);background:rgba(0,0,0,.18);border-radius:18px;padding:20px;display:flex;flex-wrap:wrap;gap:10px;align-content:center;justify-content:center}
    .us-letter{width:46px;height:56px;border:1px solid rgba(217,164,65,.3);background:rgba(255,255,255,.05);color:#fff;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:850;cursor:grab;user-select:none;touch-action:none}
    .us-letter.dragging{opacity:.35}
    ...
    .us-btn{border:1px solid rgba(217,164,65,.4);background:rgba(217,164,65,.09);color:#fff;border-radius:11px;padding:10px 16px;cursor:pointer;font:700 11px inherit}
    .us-status{min-height:18px;margin-top:13px;font-size:10px;opacity:.5}
    .us-win{display:none;margin-top:18px}.us-win.show{display:block}
    .us-win-main{font-size:30px;font-weight:900;letter-spacing:.06em}
    .us-win-main .a,.us-win-main .i{color:#e9ae50}
    .us-win-sub{font-size:11px;opacity:.5;margin-top:7px}
    /* The old coin-toss/questionnaire is not merely covered: every old element in this slide is removed from rendering. */
    #sLetters.birthday-unscramble-host > *{display:none!important;visibility:hidden!important}
  `;
  ...
  function render(){
    ...
    letters.forEach((letter,index)=>{
      ...
      el.addEventListener('dragstart',e=>{if(solved)return;dragIndex=index;el.classList.add('dragging');e.dataTransfer.effectAllowed='move';});
      el.addEventListener('dragend',()=>el.classList.remove('dragging'));
      el.addEventListener('dragover',e=>e.preventDefault());
      el.addEventListener('drop',e=>{e.preventDefault();if(solved||dragIndex===null||dragIndex===index)return;const moved=letters.splice(dragIndex,1)[0];letters.splice(index,0,moved);dragIndex=null;render();});
  ...
  function slideActive(){
    const s=document.getElementById('sLetters');
  ...
  function enter(){
    ...
    /* Hard replace: hide the entire original coin-toss/questionnaire DOM inside this slide. */
    [...slide.children].forEach(el=>{el.style.display='none';el.style.visibility='hidden';});
    reset();
